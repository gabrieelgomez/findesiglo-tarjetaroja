  ...
  module Admin
    class ProductsController < ResourceController
      ...
      def update
        ...
        variant_snapshots = snapshot_variants_from_product_params

        if @product.update(permitted_resource_params)
          set_current_store
          log_variant_changes_from_product_edit(variant_snapshots)
      ...
      def snapshot_variants_from_product_params
        return {} unless params.dig(:product, :variants_attributes).present?

        variant_ids = params[:product][:variants_attributes].values.filter_map { |va| va[:id].presence&.to_i }.uniq
        return {} if variant_ids.empty?

        variants = @product.variants.where(id: variant_ids)
        request_id = request.request_id
        variants.each_with_object({}) do |v, h|
          snap = VariantChangeLogger.snapshot(v)
          h[v.id] = snap.merge(request_id: request_id)
        end
      end

      def log_variant_changes_from_product_edit(variant_snapshots)
        return if variant_snapshots.blank? || try_spree_current_user.blank?

        request_id = request.request_id
        variant_snapshots.each do |variant_id, data|
          variant = Spree::Variant.find_by(id: variant_id)
          next unless variant&.persisted?
          ...
          VariantChangeLogger.log_from_diff(
            ...
            before: data[:before],
            before_prices: data[:before_prices],
            before_stock_items: data[:before_stock_items]
          )
        end
      end
