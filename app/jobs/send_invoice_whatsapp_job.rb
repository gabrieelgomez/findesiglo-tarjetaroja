...
class SendInvoiceWhatsappJob < ApplicationJob
  queue_as :default
  ...
  def perform(order_id, phone)
    ...
    unless order.invoice_image.attached?
      Rails.logger.error "Order ##{order.number} does not have invoice image attached"
      return
    end
    ...
    if response[:success]
      Rails.logger.info "Invoice sent successfully to #{phone} for Order ##{order.number}"
    else
